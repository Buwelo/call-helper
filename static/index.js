document.addEventListener('DOMContentLoaded', () => {
  ...
  const textarea = document.getElementById('editable-transcript');
  ...
  textarea.addEventListener('input', function () {
    ...
    lines = lines.map(line => {
      // Remove extra spaces between words and around punctuations within the line
      return line.replace(/\s+/g, ' ') // Replace multiple spaces with a single space
        .replace(/\s([.,!?;:])/g, '$1') // Remove space before punctuations
        .replace(/([.,!?;:])\s+/g, '$1 ') // Ensure single space after punctuations
